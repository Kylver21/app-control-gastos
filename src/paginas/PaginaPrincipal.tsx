...
export function PaginaPrincipal({ session, onLogout }: { session: Session; onLogout: () => Promise<void> }) {
  ...
  return <div className="min-h-screen bg-[#f8f7fb] text-[#1d1d26]">
    ...
    <main id="inicio" className="mx-auto max-w-[1440px] space-y-6 px-4 py-6 sm:px-5 lg:px-10 lg:py-9">
      ...
      <GastosRecurrentes recurrentes={storage.recurrentes} cuentas={storage.cuentas} ingresos={resumen.ingresos} onAgregar={agregarRecurrente} onEditar={(item) => { setRecurrenteEditando(item); setRecurrenteFormAbierto(true); }} onPausar={(item) => void storage.actualizarRecurrente(item.id, { activo: !item.activo })} onEliminar={(id) => void storage.eliminarRecurrente(id)} onRegistrar={storage.registrarRecurrentes} />
